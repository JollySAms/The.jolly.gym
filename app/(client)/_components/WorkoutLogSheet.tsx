...
import { NewExerciseCelebration, PRCelebration, type PRResult } from "./CelebrationScreens";
...
export function WorkoutLogSheet({ sessionId, workoutName, workoutSnapshot, onClose }: Props) {
  ...
  const [celebrationStep, setCelebrationStep] = useState<"newExercise" | "pr" | "done" | null>(null);
  const [newExerciseNames, setNewExerciseNames] = useState<string[]>([]);
  ...
  function removeSet(exIndex: number, setIndex: number) {
    ...
    if (updated.length === 0) { removeExercise(exIndex); return; }
    setExercises((prev) => prev.map((e, i) => (i === exIndex ? { ...e, sets: updated } : e)));
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const savedExIds = new Set(toSave.map(({ ex }) => ex.exerciseId));
      const stalePrescribedIds = [...savedPrescribedIds].filter((id) => !savedExIds.has(id));
      ...
      for (const result of saveResults) {
        if (result.isNewExercise) newNames.push(result.exerciseName);
        for (const pr of result.prs) prs.push({ ...pr, exerciseName: result.exerciseName });
      }

      setNewExerciseNames(newNames);
      setPrResults(prs);

      if (newNames.length > 0) setCelebrationStep("newExercise");
      else if (prs.length > 0) setCelebrationStep("pr");
      else setCelebrationStep("done");
    } catch {
  ...
  return (
    <>
        ...
        <div className="relative w-full max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <p className="text-base font-bold text-gray-900">{workoutName ?? "Workout loggen"}</p>
            </div>
          ...
          <div className="overflow-y-auto flex-1 px-4 py-4 space-y-6">
            {celebrationStep === "newExercise" ? (
              <NewExerciseCelebration
                exerciseNames={newExerciseNames}
                onContinue={() => setCelebrationStep(prResults.length > 0 ? "pr" : "done")}
              />
            ) : celebrationStep === "pr" ? (
              <PRCelebration prs={prResults} onContinue={() => setCelebrationStep("done")} />
            ) : celebrationStep === "done" ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <p className="text-4xl">👍</p>
                <p className="text-lg font-bold text-gray-900">Lekker bezig!</p>
                <p className="text-sm text-gray-500">Workout opgeslagen</p>
                <div className="flex flex-col gap-2 w-full mt-4">
                  <button onClick={onClose} className="w-full py-3 rounded-xl text-sm font-semibold bg-gray-900 text-white hover:bg-gray-700 transition-colors">Klaar</button>
                  <button onClick={() => setCelebrationStep(null)} className="w-full py-3 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-100 transition-colors">Verder loggen</button>
                </div>
              </div>
            ) : !initialized ? (
              ...
              <>
                {lastLogs && Object.keys(lastLogs).length > 0 && (
                  <button onClick={prefillFromPrevious} disabled={prefilled} className="w-full py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                  ...
                  <div key={ex.exerciseId}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        {ex.isSubstitute && (
                          <span className="text-[10px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full">vervanger</span>
                        )}
                      </div>
                      <button onClick={() => removeExercise(exIndex)} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      {ex.sets.map((set, setIndex) => {
                        ...
                        return (
                          <div key={set.id} className="grid grid-cols-[20px_2fr_1fr_1fr_24px] gap-2 items-center">
                            <span className="text-xs text-gray-500 text-center font-medium">{setIndex + 1}</span>
                            <span className="text-xs text-gray-600 truncate">{lastSet ? `${lastSet.reps}× ${lastSet.weight} kg` : "–"}</span>
                            <input type="number" min="0" step="1" placeholder="–" value={set.reps} onChange={(e) => updateSet(exIndex, setIndex, "reps", e.target.value)} className="w-full text-center text-sm py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" />
                            <input type="number" min="0" step="0.5" placeholder="–" value={set.weight} onChange={(e) => updateSet(exIndex, setIndex, "weight", e.target.value)} className="w-full text-center text-sm py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" />
                            <button onClick={() => removeSet(exIndex, setIndex)} className="flex items-center justify-center p-1 rounded hover:bg-gray-100 text-gray-300 hover:text-gray-500 transition-colors"><X size={13} /></button>
                          </div>
                        );
                      })}
                    </div>
                    <button onClick={() => addSet(exIndex)} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-900 transition-colors mt-2 py-1">
                      <Plus size={12} />
                      Set toevoegen
                    </button>
                  </div>
                ))}
                <button onClick={() => setShowPicker(true)} className="flex items-center gap-2 w-full py-2 text-sm text-gray-500 hover:text-gray-900 transition-colors">
                  <div className="w-6 h-6 rounded-full border-2 border-dashed border-gray-200 flex items-center justify-center shrink-0"><Plus size={12} /></div>
                  Oefening toevoegen
                </button>
              </>
            )}
          ...
          {/* Footer — hide during celebrations */}
          {celebrationStep === null && (
            <div className="px-4 py-4 border-t border-gray-100 shrink-0">
              <button onClick={handleSave} disabled={saving || !initialized} className="w-full py-3 rounded-xl text-sm font-semibold bg-gray-900 text-white hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {showPicker && (
        <ExercisePickerSheet onSelect={addSubstitute} onClose={() => setShowPicker(false)} excludeIds={exerciseIds} />
      )}
    </>
  );
}
